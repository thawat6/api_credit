...
from data_api.serializers import (
    ...
    StudentCourseStructureSerializer,
    StructurePreferredCourseEnrollSerializer,
    TransferringEquivalentCourseSerializer,
    TransferringEquivalentCourseCreateSerializer,
    TransferringEquivalentCourseUpdateSerializer,
    ShowUserProfileSerializer,
    AllTransferringEquivalentCourseSerializer,
    CommitteeUserSerializer,
    CommitteeUserCreateSerializer,
    EquivalentCourseSerializer,
    EquivalentCourseCreateSerializer,
    TitleProfileSerializer,
)
...
def set_user_password(request, pk):
    if request.data["password"] != request.data["confirm_password"]:
        return Response(
            {"detail": "Password and confirm paswword not match"},
...
class TitleProfileViewSet(viewsets.ModelViewSet):
    queryset = TitleProfile.objects.all()
    serializer_class = TitleProfileSerializer
    filter_backends = (DjangoFilterBackend, SearchFilter, OrderingFilter)

    def get_serializer_class(self):
        if self.request.method in ["POST", "PUT", "PATCH"]:
            return TitleProfileSerializer
        return TitleProfileSerializer


class EquivalentCourseViewSet(viewsets.ModelViewSet):
    queryset = EquivalentCourse.objects.all()
    ...
    def get_serializer_class(self):
        if self.request.method in ["POST", "PUT", "PATCH"]:
            return EquivalentCourseCreateSerializer
        return EquivalentCourseSerializer
